...
export function CriteriaRow({
  ...
  const handleWhereSave = useCallback(
    (patch: Partial<{ operator: QbColumnSelection['where'] extends infer W ? W extends { operator: infer O } ? O : never : never; value: string | null }>) => {
      ...
      onUpdate({
        where: {
          ...
          operator: (patch.operator as QbColumnSelection['where'] extends { operator: infer O } ? O : never) ?? existing?.operator ?? '=',
  ...
  return (
    <tr className="border-b border-edge transition-colors hover:bg-surface-raised/50" data-testid="criteria-row">
      ...
      <td className="px-1 py-1">
        ...
        >
          {selection.where ? `${selection.where.operator} ${selection.where.value ?? ''}` : 'Where…'}
        </button>
        <WhereEditor
          open={whereOpen}
