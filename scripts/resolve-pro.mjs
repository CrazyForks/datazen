...
export function proEntryFile(proPath = DEFAULT_PRO_DEST) {
  // Compiled Pro always points at real source; prebuilt `dist` is the fallback.
  const candidates = ['src/index.ts', 'dist/index.esm.js', 'dist/index.js'];
  for (const c of candidates) {
    const p = resolve(proPath, c);
    if (existsSync(p)) return p;
  }
  return resolve(proPath, 'src/index.ts');
}

export function writeProCodegen(dest = GENERATED_PRO_TS, { proPath = DEFAULT_PRO_DEST } = {}) {
  const entry = proEntryFile(proPath);
  const content = `/**
 * AUTO-GENERATED by resolve-pro.mjs — DO NOT EDIT MANUALLY.
 * Edition: pro (compiled into the host bundle at build time).
 *
 * The SQL Editor Pro extension is STATICALLY imported through the
 * \`@datazen/extension-sql-editor-pro\` alias (wired in \`tsconfig.paths\` and the
 * Vite \`resolve.alias\`), so it is bundled into the SAME module graph as the host.
 * It therefore shares every runtime singleton with the host — \`@codemirror/*\`,
 * the \`@datazen/extension-points\` registry, the i18n bridge — eliminating the
 * runtime instance-split that broke cross-module integrations (e.g. as-you-type
 * linting) under the old hot-plug \`hostExtensionLoader.loadFromUrl\` scheme.
 */
...
export function initProExtensions(): void {
  try {
    // Register every Pro feature into the host's shared extension registry now,
    // synchronously at startup, sharing all singletons.
    (activateSqlEditorPro)(extensionRegistry);
    console.log('[pro] SQL Editor Pro activated (compile-time bound)');
  } catch (err) {
    console.error('[pro] failed to activate SQL Editor Pro (compile-time):', err);
  }
}
`;
  mkdirSync(dirname(dest), { recursive: true });
  writeFileSync(dest, content, 'utf-8');
  console.log(`[resolve-pro] generated-pro.ts written for Pro edition (compile-time import through @datazen/extension-sql-editor-pro -> ${entry})`);
}
...
export function resolvePro(opts = {}) {
  ...
  if (edition === 'pro') {
    ...
    writeProCodegen(GENERATED_PRO_TS, { proPath: targetPath || proPath || DEFAULT_PRO_DEST });
    // Compile-time installation: no runtime `builtin-ep` asset is staged/bundled —
    // the Pro source is statically imported straight into the host Vite graph.
