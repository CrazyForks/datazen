        ...
        "batch_rename_prefix" => {
            let old_prefix = req_str(&input, "oldPrefix")
                .or_else(|_| req_str(&input, "old_prefix"))?;
            let new_prefix = req_str(&input, "newPrefix")
                .or_else(|_| req_str(&input, "new_prefix"))?;
            let keys = {
                let v = opt_string_vec(&input, "keys");
                if v.is_empty() {
                    None
                } else {
                    Some(v)
        ...
        "flush_db" => {
            let allow = input
                .get("allowFlush")
                .or_else(|| input.get("allow_flush"))
                .and_then(JsonValue::as_bool)
                .unwrap_or(false);
            crate::ops::ensure_flush_allowed(allow).map_err(DriverError::InvalidConfig)?;
