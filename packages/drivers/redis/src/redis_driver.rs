...
impl RedisDriver {
    ...
    pub(crate) async fn select_db(live: &mut RedisLiveConn, db_index: u32) -> Result<(), String> {
        with_redis_conn!(live, |conn| crate::redis_driver_on::select_db_on(conn, db_index).await)
    }
    ...
    pub async fn scan_keys_with_info(
        ...
        with_redis_conn!(&mut rc.live, |conn| crate::redis_driver_on::scan_keys_with_info_on(
    ...
    pub async fn get_key_detail(
        ...
        with_redis_conn!(&mut rc.live, |conn| crate::redis_driver_on::get_key_detail_on(conn, &key).await)
    }

    plugin_on_db!(plugin_set_string, (key: &str, value: &str, keep_ttl: bool) -> (), |conn| crate::ops_ttl::set_string_keep_ttl(conn, key, value, keep_ttl));
    plugin_on_db!(plugin_set_expire_at, (key: &str, expire_at: i64) -> (), |conn| crate::ops_ttl::set_expire_at_key(conn, key, expire_at));
    ...
    async fn test_connection_inner(
        ...
        let info: String = with_redis_conn!(&mut live, |conn| crate::redis_driver_on::info_server_on(conn).await)
            .map_err(DriverError::QueryFailed)?;
    ...
    pub async fn plugin_exec(
        ...
        if let Some(addr) = node_addr.filter(|s| !s.trim().is_empty()) {
            let plan = self.connection_plan(connection_id).await?;
            let mut conn = open_pinned_node_conn(&plan, addr.trim()).await?;
            crate::redis_driver_on::select_db_on(&mut conn, db_index)
                .await
                .map_err(DriverError::QueryFailed)?;
