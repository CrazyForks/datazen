...
//!   command — the sidebar shows an empty slot, not a red error.
//!
//! # Why replies are read via `req_packed_commands`
//!
...
//! arguments `execute_pipelined_async` passes, so the bytes on the wire, the
//! transport and cluster routing stay identical; only the error unwrapping is
...
/// carries the payload itself, and unknown future shapes are also treated as
/// errors, so a new server-side reply type can never be silently read as "the
/// key is missing".
...
/// Assemble the sidebar payload from one pipeline reply vector.
///
/// Slot order is fixed by [`build_key_info_pipeline`]; `TYPE` is read last.
/// An errored `TYPE` is reported as "attributes unreadable" (`missing: false`,
/// `type: None`) rather than `missing: true`, which would claim the key is gone.
...
/// Issue a pipeline and return the raw per-command replies (see module docs).
///
/// The outer `Err` is reserved for transport failures, i.e. a batch that never
/// happened at all; per-command errors stay inside the returned vector.
...
/// Resolve `TYPE` for the sample in [`TYPE_PIPELINE_CHUNK`]-sized pipelines.
async fn sample_types<C>(conn: &mut C, keys: &[String]) -> Result<BTreeMap<String, u64>, String>
where
    C: ConnectionLike + Send,
{
...
where
    C: ConnectionLike + Send,
{
    let limit = resolve_type_sample_limit(requested_limit);
    let dbsize = fetch_dbsize(conn).await?;
    let keys = collect_sample(conn, limit).await?;
    let type_batches = keys.len().div_ceil(TYPE_PIPELINE_CHUNK);
    let result = TypeDistribution::from_sample(sample_types(conn, &keys).await?, dbsize);
    tracing::info!(
...
/// Key attributes for the sidebar, in a single pipeline.
pub async fn key_object_info<C>(conn: &mut C, key: &str) -> Result<KeyObjectInfo, String>
where
    C: ConnectionLike + Send,
{
