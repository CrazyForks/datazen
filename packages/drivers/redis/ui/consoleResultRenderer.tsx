...
export function ConsoleResultView({ item }: { item: ConsoleResultItem }) {
  ...
  if (item.resultType === "nil") {
    return (
      <span className="text-zinc-500 italic font-mono text-sm">(nil)</span>
    );
  }
  ...
  // scalar
  return (
    <span className="text-zinc-200 font-mono text-sm">{item.value}</span>
  );
}
        ...
        <tbody>
          {items.map((item, i) => (
            <tr key={i} className="border-t border-zinc-800 first:border-t-0">
              <td className="px-3 py-1 text-zinc-500 w-10 text-right select-none">
                {i}
              </td>
