...
### 坑 21：把候选原样铺到画布上，等于没有预测

一个没有声明外键的库，引擎两侧各猜一遍就是双份的线：同名标签成簇、从画布这头跨到
那头，而且都落在表节点的垂直中点上，看着像在说同一行。用户的第一反应是关掉这个面板
—— 猜得再准也没用。

**规避**（已落在 `src/windows/connection/er/buildErGraph.ts`）：

- 对称候选合并：`a.col → b.col` 与 `b.col → a.col` 是同一条未知链接看两次，留高分方向
- 歧义候选降权而不隐藏：虚线更稀（`2 5`）、整条线（含箭头）淡到 30%、**不打标签**；
  这类库的线本来就几乎全是推测出来的，全隐掉等于没有图
- 连线锚在**列**上：连接点按列渲染，标签因此是冗余的；一簇相同标签正是画布读不懂的原因
- 布局用分层排布（dagre，引用方向左→右），而不是表名字母序或手排网格：相关表落在
  相邻层级，线就不用横穿整张画布（合并 fk-prediction 时上游已将早期连通性网格方案
  替换为分层布局，本行随实现同步更正）
- 统计数字必须与画出的线条数一致（同样先做对称合并）
