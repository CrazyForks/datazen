...
| 档位 | 分数 | UI 行为 |
| --- | --- | --- |
| **Confirmed** | 声明式 FK | 实线；自动 JOIN（现状不变） |
| **High** | ≥ 0.80 | **虚线 + 徽标**；自动 JOIN，用户可一键移除 |
| **Medium** | 0.50 ~ 0.80 | 只在「建议关联」列表里出现；**不自动 JOIN**，点一下才连 |
| **Low** | < 0.50 | 不展示（避免噪声淹没画布） |

**关键**：High 档虽然自动 JOIN，但**必须与声明式 FK 视觉可区分**（不同线型 + 徽标 +
悬浮说明理由）。用户一眼要能看出「这条是猜的」。
...
全部是**注入元数据的纯函数**，不碰 IPC——与 `relationMetadataSource.ts` 同样的可测形态。
