...
- 在全新 Tester worktree 的 `392e851aa` 修复态独立复测。以“移除 compact overflow 的 `types.sample` 子树”做单项反向变异，只运行该回归用例，结果 **1 failed / 48 skipped**：`expected null not to be null`（`redisContextBar.test.tsx:746`）。由此确认该断言独立承重；变异现场已还原。
...
- 独立结论：修复通过。本轮没有运行 Tauri GUI / 真 Redis；compact 实际布局旅程留待 R 回归，不影响组件单测对本缺陷的闭环。
