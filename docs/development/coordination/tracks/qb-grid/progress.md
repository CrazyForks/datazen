# Track: qb-grid (Phase 3: Object Tree + Criteria Grid)

## 目标
实现数据库对象树面板和条件配置网格（类 Excel），提供表选择和查询条件配置界面。

## Scope
- `src/components/query-builder/ObjectTreePanel.tsx` — 数据库对象树
- `src/components/query-builder/CriteriaGrid/CriteriaGrid.tsx` — 条件网格容器
...
2. 对象树可展开/折叠，支持拖入表到画布
3. CriteriaGrid 可配置字段/排序/聚合/条件
4. WHERE 编辑弹窗功能完整
