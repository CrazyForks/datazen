...
- **四条更正逐条核验**（progress.md「修复轮 round-1」区）：更正1（L513-519，口径 B 引用以复算 **85.57/88.64** 为准、自报 86.25/88.74「本节起作废」——权威声明在位）；更正2（L525-526，`invokeRename:296-311`/`invokeDeleteKey:314-325` 归属纠正 + 指向 T-11 补测）；更正3（L529-532，「9 语言孤儿副本」不实陈述作废，附 T-7 实测 0 命中与 i18n-sync-check 0 issue 证据）；更正4（L534-538，「11 个调用点 / 8 类动作」统一口径）。历史原文全部保留、新节声明权威 ⇒ 追加式更正（偏差③）可接受。
- **README 索引**：6 行状态列 round-1 已同步 `待复测`，本轮翻 `已修复` 并追加 BUG-007 行。
- **round-2 覆盖率独立复算**（同 v8 配置、`--coverage.all=false`、按 `git diff 8981d3078..HEAD` 文件集对 coverage-summary.json 求 Σcovered/Σtotal）：口径 A(15) **88.84% stmts / 91.20% lines**；口径 B **92.43% / 95.43%**（与 round-1 补测后 92.30/95.34 同量级 +0.13/+0.09pp，来自本轮新测）；≥80% 硬线余量充足，更正1 结论无回归。
