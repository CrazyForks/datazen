...
- 状态: **TEST_DONE（第 4 轮复测通过，Tester ecf1171f-aaa7-424e-8271-8114bbfa98c4，2026-09-23）**（**BUG-008（高）+ BUG-009（低）均翻「已修复」**，Bug 循环 4/5 → 待协调者收口合入）（第 4 轮复测历史：文件面审计 `3e25a3c0f..b7ef8a009` 恰 7 文件全落许可面、禁改面反向 grep NONE；BUG-008 基线 8/8 绿 · 0 skipped，变异矩阵 (i) 语义等价故绿（已归因：修复前偏差⑥ 唯一机制是被删的 `onUpdateSelectedKey`，`await onSelectKey` 前移/后移为等价变换）、(ii) 守卫后加回无条件写入 **5 failed/3 passed** 红、(ii-a) 忠实回退 `3e25a3c0f` 亦红、(iii) 守卫前直改选中键 亦红 ⇒ 因果面钉住；不变式实测 三路径 保存目标键===面板键===选中键 成立；裁定独立验证 **成立**（4 个 `onSelectKey` 调用点 + 删除/批量路径逐一核对，无合法路径丢失选中键更新）；BUG-009 `wc -l`=795（≤800）、守卫 3 行 md5 逐字节未动、`cbecba255` = `6 15` 纯注释；四门 G1 `60 files/564 passed/0 skipped` · tsc 0 · build `✓ built in 4.83s` 0 · boundaries `1474/0/4` 0；覆盖率 方法经 round-3 态逐位复现自证（A 88.88/91.22、B 92.43/95.43），like-for-like 旧口径 B **92.43/95.43 逐位相同（零回归）**，本轮全量口径 B 85.95/89.03 · A 84.34/86.93 均 ≥80%；断言纪律零文案字面量/零几何反查/无 vacuous/skip 零残留）。（第 3 轮复测判定 TEST_FAILED 历史：BUG-007 复测通过翻「已修复」，但追出新 BUG-008（高）+ BUG-009（低）；round-2/1 历史见下文。）
...
- **零英文文案字面量** ✅：对 `git diff 3e25a3c0f..HEAD -- __tests__/` 的新增断言行做正则扫描
  （`toBe/toEqual/toHaveBeenCalledWith/toContain` 后接 ≥5 字母的纯英文串）⇒ **零命中**。
  新增断言全部落在 `data-*` 属性（`data-selected-key` / `data-string-dirty`）、i18n key（`useI18n` stub 为 identity `t`）
  与测试自造键名/数据（`'user:1'` / `'user:renamed'` / `'draft'`）上。
- **零几何反查** ✅：两文件对 `getBoundingClientRect` / `offsetWidth` / `offsetHeight` / `clientX` / `clientY` /
  `getComputedStyle` / `scrollIntoView` 全部零命中（exit 1）。
- **零 vacuous 断言** ✅：新增断言共 7 行，均为具体取值断言（`toBe('user:1')`、`toHaveBeenCalledWith('sess-r3', 0, 'user:1', 'draft')`）；
  唯一两处 `expect(item).toBeTruthy()`（`:200` / `:206`）经 diff 核验为**round-4 之前既有**的 harness 前置守卫（非本轮新增），
  且其后紧跟 `item.action()` 真实驱动菜单 —— 非空断言。**关键：本轮变异矩阵 (ii)/(ii-a)/(iii) 三项实测转红**
  ⇒ 新断言**可证伪**，实证非 vacuous。
- **skip 归零** ✅：两文件 `describe.skip` / `it.skip` / `.only` / `it.todo` / `xit` / `xdescribe` **全部零命中**（exit 1），
  与 G1 实测 `0 skipped` 一致 —— D 组转正真实生效（非「跳过仍绿」）。

### 台账审阅 ✅

- **BUG-008**：标题 + `## 复现步骤` / `## 期望 vs 实际` / `## 根因` / `## 与 BUG-007 的关系` / `## 修法建议` /
  `## 影响范围` / `## 修复记录（coder round-3）`（含 `### 改动` / `### 与简报伪码的实质偏差` / `### 断言同步改写` /
  `### D 组 unskip 结果` / `### 四门禁` / `### 变异反向验证`）结构完整。
- **BUG-009**：标题 + `## 事实` / `## 与自报的差异` / `## 影响范围` / `## 修法建议` / `## 修复记录（coder round-3）`
  （含 `### 实测行数` / `### 改动内容` / `### 不回归核对` / `### 与本条台账事实的关系`）结构完整。
- **历史未删改** ✅：`git diff 3e25a3c0f..HEAD -- progress.md` 的**唯一删除行**就是头部状态行本身（被替换），
  round-1/2/3 全部历史小节与复验记录**逐字保留**、零删除。
- **头部状态**：当前为 `**READY_FOR_TEST（修复轮第 3 回合完成）**` —— 与本轮起点（修复轮第 3 回合交付）**相符**，终判时按协议翻转。

### 发现（非阻塞，未登记 bug）

`bugs/README.md` 索引表中 `BUG-008` / `BUG-009` 两行的「状态」列仍写 **`待修复`**，
而两 bug 正文的 `**状态**` 行已是 `待复测（round-3 修复后）` ⇒ **索引与正文不同步**。
→ 判定：**台账一致性瑕疵（低）**，不构成功能缺陷，且**属本 Tester 可写的许可面**（`bugs/README.md` 索引行）
  ⇒ 本轮**就地修正**为 `待复测（round-3 修复后）`，终判翻 `已修复` 时再随状态行同步，不另立 bug 文件。

## R4-8 第 4 轮终判 · `TEST_DONE`（第 4 轮复测通过）

Tester `ecf1171f-aaa7-424e-8271-8114bbfa98c4`（全新实例，只测不修）· 复测 HEAD `b7ef8a009` ·
Bug 循环 **4/5**（无新登记 bug）。一动作一 commit，commit 链：

1. `cda32f425` 文件面审计（7 文件、禁改面 NONE）
2. `170d866c5` BUG-008 核心复验（变异矩阵 + 不变式）
3. `6b6c1ede3` 裁定独立验证（成立）
4. `8ffb21abb` BUG-009 复验（795 行、守卫逐字节未动）
5. `407cce437` 回归四门
6. `4a91b3b53` 覆盖率回归（like-for-like 零回归）
7. 本笔：断言纪律 + 台账审阅 + **终判收官**（两 bug 翻「已修复」+ 头部翻 `TEST_DONE`）

### 交付项判定汇总

| 项 | 结果 |
| --- | --- |
| 文件面审计（7 文件 / 禁改面 NONE） | ✅ 通过 |
| BUG-008 核心复验（前提 + 8/8 绿 + 变异矩阵） | ✅ 通过 |
| BUG-008 不变式（保存键===面板键===选中键） | ✅ 三路径成立 |
| 协调者裁定独立验证 | ✅ **裁定成立** |
| BUG-009（795 ≤ 800 / 纯注释 / 守卫未动） | ✅ 通过 |
| 回归四门（G1 564/0skip · G2 0 · G3 0 · G4 1474/0/4） | ✅ 全绿 |
| 覆盖率（旧口径 B like-for-like 92.43/95.43） | ✅ **零回归**，全量口径 ≥80% |
| 断言纪律（零文案字面量 / 零几何反查 / 无 vacuous / skip 零残留） | ✅ 通过 |
| 台账审阅（结构完整 / 历史未删改 / README 同步） | ✅ 通过（并就地修正 README 索引状态滞后） |

### 最终状态

- **`BUG-008`（高）→ `已修复`**（round-4 复测通过）；**`BUG-009`（低）→ `已修复`**（round-4 复测通过）。
- 全轨 **9 条 bug 全部 `已修复`**（BUG-001~009），无 `待修复` / 无 `待复测`。
- 未触碰禁止面；复测期内所有变异注入与临时探针均 `git checkout HEAD --` / 删除还原，
  `git status --porcelain` 收口为**净**。
- **停止并汇报**（无新 bug、无需回退）。
