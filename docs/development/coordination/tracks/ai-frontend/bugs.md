...
- **Suggested Fix**: Store the `sessionKey` on the `chatSession` object (set during `initChatSession`) and use it during persistence in `handleStreamChunk`.
...
- **Suggested Fix**: Pass `dbSessionId` and `database` to `initChat(dbSessionId, database)` in the `useEffect` at line 96-98.
...
- **Suggested Fix**: Update the type to `initChatSession: (connectionId?: string, dbSessionId?: string, database?: string) => void`.
