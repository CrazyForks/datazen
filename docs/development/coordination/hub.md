...
- **Wave 3**（2026-09-23）：
  ...
  - **合流后首次全量 drivers 跑出 12 例红 —— 全部是 D 拆分后的 testid 迁移未同步，语义零丢失**（这是「两轨各自全绿、合流才暴露」的典型）：`redis-refresh`→`redis-tree-refresh`、`redis-create-key`→`redis-tree-create-key`、`redis-no-ttl-only`→`redis-tree-chip-no-ttl`（三者生产码语义完好，刷新按钮接的正是 E 加了守卫的 `handleRefresh`）；另 1 例是 D 自己的 `keyBrowserModuleSplit.test.ts` 400 行推荐上限被 E 的守卫增长顶破（`RedisWorkbench.tsx` 408，HARD 800 未破）。**教训**：跨轨拆分/改名 testid 时，必须在 `progress.md` 登记「旧名 → 新名」对照，否则消费轨的旅程测试会在合流时集体失效。
