...
- **Pure functions / IPC wrappers**: move the implementation down into `@datazen/driver-sdk` (one single implementation in the whole repository; the host path keeps only a thin re-export). Existing precedents: `driverSettings`, `resolveEditorFontFamily`, `ipc/driverCommands`, `ipc/fileCommands`, `nativeContextMenu`.
...
When a new host capability is required, **do not import it from the host**; sink it down or add a bridge following the procedure in contract document section 2.5.
...
- **Translations are provided and self-registered by the driver package**: dictionaries live in `locales/` (one file per language, keys carrying the driver's own prefix such as `redis.*` / `mongo.*`); the pure side-effect module `locales/index.ts` calls `registerTranslations`, hooked up via a single `import '../locales';` side-effect line in the driver UI entry module (the first UI module actually imported by `generated.ts`, e.g. `ui/shared/meta.ts`). **This self-registration chain lands concurrently on the `i18n-drivers` track** (the host-side `DRIVER_LOCALES` aggregation codegen is deleted by the same track); do not rely on the legacy host aggregation when adding new driver translations.
- Driver-side `t()` keys are plain `string`s — there is no compile-time `I18nKey` checking; translation completeness is enforced by `node scripts/i18n-sync-check.mjs` scanning each package's `locales/` (the driver-directory scan is likewise added by the `i18n-drivers` track). During development only edit your package's `en.ts` (the single source of truth).
